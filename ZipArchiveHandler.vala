...
/*
    To compile: valac ZipArchiveHandler.vala  --pkg gio-2.0 --pkg libarchive
    Usage: ./ZipArchiveHandler zipped_content.zip

    The first time you run it, it will create a folder named .~lock.appname.zipped_content.zip

    The second time it is run, it will package everything to zipped_content.zip, deleting the tmp folder
*/
...
public class ZipArchiveHandler {
    ...
    private const string UNARCHIVED_PREFIX = ".~lock.appname.";
     ...
     */
    public void save_file () throws Error {
    ...
    private static void extract (File gzipped_file, File location) {
        ...
        if (archive.open_filename (gzipped_file.get_path (), 10240) != Archive.Result.OK) {
            critical ("Error opening %s: %s (%d)", gzipped_file.get_path (), archive.error_string (), archive.errno ());
            return;
        }
    ...
    private static void compress (File location, File gzipped_file) {
    ...
    public static int main (string[] args) {
        ...
        var zipped_file = new ZipArchiveHandler (infile);
        if (zipped_file.is_opened ()) {
            print ("Saving to archive\n");
            zipped_file.save_file ();
            zipped_file.clean ();
        } else {
            zipped_file.open ();
        }
